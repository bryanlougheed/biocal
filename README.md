...
The Python version is the version that is currently receiving active updates and has been heavily optimised using machine code. The Matlab version is no longer updated but still included here (biocal_matlab.zip) for legacy purposes. Note that because the Matlab version has not been similarly optimised, it will be significantly slower and use many more GB of RAM. I no longer have a Matlab license so you are on your own if you run into problems.
