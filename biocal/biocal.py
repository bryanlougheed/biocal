...
def _calc_p14ctt(curvef14, curveerr):
	"""numba jit machine code calculation of p14cTt to avoid S*S memory bloat."""
	nrows = curvef14.size
	p14cTt = np.zeros(nrows)
	...
	for i in prange(nrows):
		...
		for j in range(nrows):
			err_j = curveerr[j]
			diff = f14_i - curvef14[j]
			row_sum += (inv_sqrt_2pi / err_j) * np.exp(-(diff**2) / (2.0 * err_j**2))
		p14cTt[i] = row_sum
...
def _calc_pcal_fused(ppri, abus, p14cTt, curvef14, Adet_f14, sigdet_f14, nrows, S):
	"""
	Computes the final pcal array dynamically using kernel fusion,
	avoiding the S x S (or nrows x S) memory bloat entirely.
	"""
	N = ppri.size
	pcal = np.zeros(S, dtype=np.float64)
	inv_sig_sqrt_2pi = 1.0 / (sigdet_f14 * np.sqrt(2.0 * np.pi))

	# Iterate through each sliding window 't'
	for t in range(nrows):
		...
		# Pass 1: Accumulate unnormalised denominators for row 't'
		for k in range(rem):
			j = t + k
			p_val = ppri[k] * abus[j]
			sum_P_unnorm += p_val
			sum_W_unnorm += p_val * p14cTt[j]

		# Pass 2: Calculate hdet[t] using the normalised W weights
		hdet_t = 0.0
		if sum_W_unnorm > 0.0:
			for k in range(rem):
				j = t + k
				w_val = (ppri[k] * abus[j] * p14cTt[j]) / sum_W_unnorm
				hdet_t += w_val * curvef14[j]

		# Pass 3: Calculate phdet[t] and accumulate directly into the global pcal
		if sum_P_unnorm > 0.0:

			diff = hdet_t - Adet_f14
			phdet_t = inv_sig_sqrt_2pi * np.exp(-(diff**2) / (2.0 * sigdet_f14**2))

			for k in range(rem):
				j = t + k
				# p_val_norm is the equivalent of the fully normalised pmat[t, j]
				p_val_norm = (ppri[k] * abus[j]) / sum_P_unnorm
				pcal[j] += p_val_norm * phdet_t

	return pcal


def _hpdcalc(values, probs, valres, siglevel):
	"""
	...
	B.C. Lougheed 2016, 2022 (Matlab), 2024 (numpy python)
	"""
...
def biocal(Adet, sigdet, calcurve, yeartype, sar, bd, brok=0, abu=[], res=[]):
	...
	yeartype : string
		Specify how to handle years (not case sensitive):
		'Cal BP' or 'BCE/CE'
	...
	biocal version 2.0
	B.C. Lougheed, Original function (v. 1.0) in Matlab, Jun. 2021.
	Matlab ported to python/numpy/numexpr by B.C. Lougheed, Nov. 2024.
	Further optimised in Sep. 2026 using numba jit, by B.C. Lougheed with help of Google Gemini Pro
	"""
	...
	if abu.size == 0:
		...
		try:
			if yeartype.lower() == 'bce/ce':
				abu[:, 0] = (1950 - abu[:, 0])
			abu = abu[np.argsort(abu[:, 0])]
			abus = np.interp(curvecal, abu[:, 0], abu[:, 1])
			abus[curvecal < abu[0, 0]] = abu[0, 1]  
			abus[curvecal > abu[-1, 0]] = abu[-1, 1]  
		except Exception:
	...
	Adet_f14 = np.exp(Adet / -8033)  # copy so as not to affect the user's workspace
	sigdet_f14 = Adet_f14 * sigdet / 8033

	N = ppri.size
	nrows = te - ts + 1

	# trim cal curve to match shape
	ind = (curvecal >= curvecal[ts]) & (curvecal <= curvecal[ts] + nrows + N - 2)
	curvef14 = curvef14[ind]
	curveerr = curveerr[ind]
	curvecal = curvecal[ind]

	S = curvecal.size  
	nrows = min(nrows, S)  # Safety bound to prevent indexing beyond the curve
	...
	p14cTt = _calc_p14ctt(curvef14, curveerr)

	### Equation 7 & 8 in manuscript: calculate hdet, phdet, and construct final pcal
	pcal = _calc_pcal_fused(
		ppri, 
		abus, 
		p14cTt, 
		curvef14, 
	...
	p95_4 = _hpdcalc(calprob[:, 0], calprob[:, 1], 1, erf(2 / np.sqrt(2)))
	#p95 = _hpdcalc(calprob[:, 0], calprob[:, 1], 1, 0.95)
	p68_2 = _hpdcalc(calprob[:, 0], calprob[:, 1], 1, erf(1 / np.sqrt(2)))

	# convert output to BCE/CE if requested
	if yeartype.lower() == 'bce/ce':
		medage = (medage - 1950) * -1
		calprob[:, 0] = (calprob[:, 0] - 1950) * -1
		p95_4[:, 0:2] = (p95_4[:, 0:2] - 1950) * -1
		#p95[:, 0:2] = (p95[:, 0:2] - 1950) * -1
		p68_2[:, 0:2] = (p68_2[:, 0:2] - 1950) * -1

	return p95_4, p68_2, calprob, medage

def biocal_plot(p95_4, p68_2, calprob, medage, yeartype):
	"""
	biocal_plot(p95_4, p68_2, calprob, medage, yeartype)

	Plots the output from the biocal function.
	"""

	if yeartype.lower() == 'cal bp':
		yeartype = 'Cal yr BP'
	elif yeartype.lower() == 'bce/ce':
		yeartype = 'BCE/CE'
	else: raise ValueError('yeartype input must be Cal BP or BCE/CE')

	# 1. Shade ALL 95.4% intervals first (wider/lighter background)
	for row in p95_4:
		x_low, x_high = min(row[1], row[0]), max(row[1], row[0])
		mask = (calprob[:, 0] >= x_low) & (calprob[:, 0] <= x_high)
		plt.fill_between(
			calprob[mask, 0], calprob[mask, 1], color="skyblue", alpha=0.4
		)

	# 2. Shade ALL 68.2% intervals on top (inner/darker core)
	for row in p68_2:
		x_low, x_high = min(row[1], row[0]), max(row[1], row[0])
		mask = (calprob[:, 0] >= x_low) & (calprob[:, 0] <= x_high)
		plt.fill_between(
			calprob[mask, 0], calprob[mask, 1], color="skyblue", alpha=0.8
		)

	# Re-plot the black outline curve on top so it stays crisp
	plt.plot(calprob[:, 0], calprob[:, 1], linewidth=1)

	# Get and plot age text
	# Format 95.4% block
	p95_first_line = (
		f"95.45% HPD: {round(p95_4[0,1])} - {round(p95_4[0,0])}"
		+ (f" ({p95_4[0,2]:.3f})" if len(p95_4) > 1 else "")
	)
	p95_block = "\n".join(
		[p95_first_line]
		+ [
			f"  {round(row[1])} - {round(row[0])} ({row[2]:.3f})"
			for row in p95_4[1:]
		]
	)
	...
	# Format 68.2% block
	p68_first_line = (
		f"68.27% HPD: {round(p68_2[0,1])} - {round(p68_2[0,0])}"
		+ (f" ({p68_2[0,2]:.3f})" if len(p68_2) > 1 else "")
	)
	p68_block = "\n".join(
		[p68_first_line]
		+ [
	...
	text_content = (
		f"Median age: {round(medage)} {yeartype}\n{p95_block}\n{p68_block}"
	)
	...
	plt.title('biocal calibration result')
...
if __name__ == '__main__':
	...
	abundance = (abundance +1) / 2 # make the abundance values between 0 and 1, suitable for biocal
	...
	res = [650, 40] # constant reservoir effect. res effect +/- uncertainty
